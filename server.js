...
app.get('/api/licitacoes', authenticate, async (req, res) => {
    try {
        ...
        if (tableCheck) {
            console.error('[LICITACOES] Tabela não acessível:', tableCheck);
            return res.status(500).json({ error: 'Tabela licitacoes não encontrada', details: tableCheck });
        }
        ...
        if (mes && ano) {
            ...
            console.log(`[LICITACOES] Filtro: data entre ${startDate} e ${endDate}`);
        ...
        res.status(500).json({ error: 'Erro interno ao buscar licitações', details: err.message });
    }
});
